...
- Closed the remaining Windows-relevant v5.0.6 Temporary Chat personalization gap. Tool-capable turns now prove the configured connector before selection and can transactionally change only the owned personalization state when required, with bounded cleanup and rollback on failed proof. English and Simplified-Chinese labels are fast paths rather than the authority: the structural fallback was live-proven against the maintainer's Japanese Roxy ChatGPT UI (`パーソナライズ`), where it returned `personalization-already-enabled` from real `Codex Native3` catalog evidence; the project-native source `browser verify-connector` command then completed successfully on the same profile.
- Fixed a remaining effort-picker exit-animation race from the v5.0.6 tail audit. A still-rendered slider is ignored when its owner control already reports `aria-expanded=false` or `data-state=closed`; capability detection reopens the owner before reading the reasoning range instead of racing a stale slider out of the DOM.
- Localized known successful MCP verification/Doctor runtime messages in the Chinese Launcher while preserving warnings, failures, and unknown backend diagnostics verbatim. Connector names are substituted literally after JSON decoding, so diagnostic identities containing replacement metacharacters or escaping remain exact.
- The final v5.0.6 tail audit did not duplicate behavior already present in the 3.0.41-3.0.43 line: descriptor-v3 native target ownership, structural response errors, final `Stopped thinking` handling, browser diagnostics, Windows lifecycle recovery, and interrupt-hook `allowAbsent` support are already aligned. AsterBridge intentionally retains the proven 10-second Web `wait_agent` polling contract instead of upstream's later 30-second interval.
- U5-W4 validation passes its focused personalization/Browser Worker/capability suite at **107 pass / 0 fail**, focused Launcher localization at **3 pass / 0 fail**, final Core at **45 / 45 deterministic batches**, and final Launcher at **230 pass / 0 fail / 2 expected platform skips**, with root/Launcher TypeScript and the production renderer build green. The live source `browser verify-connector` gate also completed exit 0 against the configured Roxy profile.
...
- Selectively audited upstream **v5.0.7** as U5-W5 instead of importing its wider lifecycle rewrite. ChatGPT account capability detection now records `extraHighAvailable` independently from `proAvailable`; model publication exposes Extra High only when the effort slider proves that tier. A real authenticated Japanese Roxy probe on the maintainer account returned **Sol=true / Extra High=true / Pro=false**, proving the previous Sol/Pro shortcut could hide a genuinely available Extra High route.
- Compaction replay now protects the newest cumulative checkpoint before trimming ordinary history, and retired `request_` / turn-binding handles are scrubbed only from exact JSON string values. Object keys and native tool-call identifiers are never rewritten as a side effect of replay sanitization.
- Browser terminal-state detection now recognizes the current localized `Stopped thinking` UI for Japanese, Simplified Chinese, Traditional Chinese, and Korean in addition to English while retaining structural/current-turn scoping. This closes the Japanese Roxy gap without making localized strings the primary completion authority.
- Added exact trace-level orphan cancellation for Launcher-owned browser tabs. An expired automatic tab asks the daemon to cancel only its `traceId`, which settles the matching browser session, MCP broker ownership, and structured compaction owner before the Launcher destroys the tab. Concurrent traces remain untouched, duplicate sweeps coalesce, failed cancellation keeps the lease visible/retryable, and targeted structured-compaction cancellation waits for registered physical cleanup.
- Hardened the reversible Codex Interrupt hook against native TOML reordering. Unrelated foreign tables may now be inserted between the managed Interrupt definition and its trust-state table, but complete-document TOML comparison still rejects changed timeout/hash fields, extra owned-state descendants, appended hooks, reordered Interrupt groups, or marker changes.
- Launcher ChatGPT session verification now distinguishes real signed-out evidence (`401`, guest session, expired session) from transport/inspection failures. Proxy/network errors, timeouts, HTTP 5xx, HTML/redirect responses, invalid JSON, and renderer inspection failures surface as bounded diagnostic errors instead of being misreported as “Sign in to ChatGPT”; raw backend/proxy error text is not reflected into the UI.
- U5-W5 is **DONE**. Final `bun run verify` on Windows passes all **45 / 45 deterministic Core batches**, Launcher tests at **233 pass / 2 environment skips / 0 fail**, root and Launcher TypeScript, Vite production build, relocatable runtime bundle construction, third-party notice generation for **110 runtime packages**, and release smoke `RELOCATABLE_RUNTIME_SMOKE_OK`. The two process-style Codex catalog refresh tests use a 15-second test-only budget after repeated Windows runs completed at roughly 4.4-4.8 seconds; production runtime timeouts are unchanged.
...
- Extended that ownership boundary through the persistent Launcher helper. The helper now advertises a `physical-settlement` protocol feature and emits a separate `settled` frame after its turn `finally` cleanup; daemon-side `result` / `error` resolves only the logical browser outcome, while `physicalSettlement` remains pending until that `settled` acknowledgement arrives. Helper exit between those boundaries now fails settlement instead of pretending the launcher-owned surface was released.
- Preserved `ChatGptBrowserWorker.run()` as the public/test-injection execution boundary. Production runs register their physical settlement alongside the returned logical promise, while test/extension fakes that replace `run()` retain the previous behavior with logical completion as their settlement fallback. This avoids silently bypassing Full Harness browser fakes while still exposing true helper cleanup ownership in production.
- Re-locked retained-conversation serialization after introducing owned browser runs: the next turn for one retained Roxy/system-browser conversation cannot start its physical worker before the prior logical turn tail settles. Combined browser-worker, Full Harness, Launcher-helper, retained-compaction, and lifecycle validation passes **169 pass / 0 fail / 8 existing Windows skips / 895 assertions**, with TypeScript PASS.
- Added the Launcher-side retained-conversation lease prerequisite. Embedded task tabs can now be retained under a validated `conversationKey`, rebound by a later trace only when the exact ready tab matches, and released explicitly through the authenticated control channel. `requireRetainedConversation` fails closed with typed `409 retained_conversation_unavailable` evidence instead of allocating a fresh tab, retained tabs expire after the bounded lease TTL, and an idle retained tab may be evicted to preserve the five-surface account-safety ceiling. Launcher browser-host/control-server coverage passes **53/53** with TypeScript PASS.
- Connected that lease result to the persistent helper without precompiling the wrong prompt. The helper now negotiates `prompt-selection`, leases the Launcher surface first, emits `prepared_selected(reused)`, and waits for the daemon to compile exactly `prepare` or `prepareResume` before the browser turn proceeds. Launcher is now included in ordinary retained-conversation eligibility, same-conversation helper turns serialize behind the prior turn, and epoch retirement releases the retained Launcher tab through the authenticated control channel. Structured Launcher compaction remains intentionally disabled until MCP progress/completion-fence forwarding is present. The widened focused gate passes **260 / 0 / 8 existing Windows skips / 1058 assertions**, TypeScript PASS, and the Node helper bundle builds successfully.
- Backported the missing cross-process MCP progress contract for Launcher turns. Daemon-owned `ChatGptExternalTurnProgress` revisions are mirrored into the helper, helper observation of a tool boundary is acknowledged back to the daemon, and completion-fence `begin` / `commit` calls round-trip to the owning broker before the helper may accept DOM completion. Invalid or regressing progress frames fail closed at the mirror boundary, progress forwarding is scoped to the exact active turn and cancelled on settlement, and protocol failures abort the helper turn instead of silently degrading to DOM-only health. Focused Launcher/helper + Full Harness + lifecycle coverage passes **74 / 0 / 8 existing Windows skips / 440 assertions**, with dedicated progress/fence regression tests and TypeScript PASS.
- Enabled structured retained compaction for the Launcher host now that the prerequisite lease, prompt-selection, physical-settlement, and MCP progress/fence protocols are present. Active-source compaction waits the source session's `physicalSettlement` before starting the dedicated handoff, and the handoff registers/awaits the worker's true physical settlement rather than treating its logical browser result as proof that `/v1/turn/end` has released ownership. Adapter-level coverage proves a Launcher compact request reuses the exact retained conversation, submits one structured control handoff, returns the canonical checkpoint, and retires/releases the retained source. Focused retained-compaction + Full Harness + Launcher-helper + lifecycle validation passes **83 / 0 / 8 existing Windows skips / 497 assertions**, with TypeScript PASS.
- Hardened Full-mode tunnel startup/monitoring against the false-green MCP state seen in bridge `502` incidents. `/healthz` and `/readyz` are no longer sufficient proof of a usable tunnel: the Launcher also inspects bounded local MCP diagnostics, treats recent internal `502` failures for `initialize` / `tools/call` as fatal evidence, and refuses to adopt or newly monitor a tunnel until that MCP transport gate passes. Runtime inventory remains authoritative for a proven stopped alias but can no longer overwrite missing MCP evidence with a generic `ready` flag. The diagnostics base URL is reset on each acquisition so a new/adopted runtime cannot inherit a stale endpoint. Runtime-supervisor + runtime-host + update coverage passes **105 pass / 0 fail / 1 platform skip**.
- Added the native target identity prerequisite for exact Launcher page recovery. Browser descriptors are now v3 and publish a one-to-one `surfaceId -> CDP targetId` map for the home surface and every leased task tab; page acquisition resolves ownership from browser-process target metadata instead of executing JavaScript inside candidate renderers. A newly leased tab rewrites the descriptor before the helper connects, and stale v2 descriptors fail closed with an explicit restart requirement. Focused CLI/descriptor/helper coverage passes **25/25**, Launcher browser-host/control coverage passes **53/53**, and TypeScript PASS.
- Completed bounded same-page recovery for accepted Launcher turns. Submission acceptance, Bigger Context acknowledgements, and the terminal response loop now wrap DOM observations in a **5 s** probe; after a proven observation failure AsterBridge closes the stale Playwright/CDP connection first, then reconnects to the exact descriptor-v3 target for the already-leased task tab. Recovery is capped at **2** rebinds, never re-presses Send or allocates a second ChatGPT conversation, and refuses replacement entirely if the stale connection cannot be closed. The widened browser/Full-Harness/compaction/lifecycle gate passes **185 pass / 0 fail / 8 existing Windows skips / 959 assertions**, TypeScript PASS, and the Launcher renderer/build gate passes.
- Closed the remaining Windows-relevant Launcher lifecycle/update code delta without importing the upstream Zero Risk/Native-subagent/Linux-updater redesign. Ownership journals older than the current OS boot no longer block startup after Windows PID reuse; tunnel recovery force-replaces a runtime that monitoring has already judged unhealthy instead of re-adopting a stale `ready` inventory record; fatal local MCP `502` evidence consumes the monitor failure budget immediately; reconnect resets the discovered diagnostics endpoint; managed launcher upgrades refresh account/model capability detection instead of carrying a stale probe forward. Setup rollback now follows a symlinked Codex `config.toml`, verifies that the link still points to the captured target, and restores the target atomically without replacing the link or tightening its parent directory. Focused runtime-supervisor/runtime-host/update coverage passes **109 pass / 0 fail / 2 environment/platform skips**, with TypeScript PASS.
- Completed the required short Windows/Roxy production-style U5-W2 live gate on the 3.0.42 candidate without modifying the installed 3.0.39 production config. A live account probe returned **Sol=true / Pro=false**; a real RoxyBrowser Temporary Chat smoke selected **High**, sent through the current composer/DOM path, and returned `CODEX WEB GPT READY`. An isolated Full/MCP runtime on **127.0.0.1:17842** then reported tunnel healthy/ready and ran native Codex through two same-owner Web turns, one structured retained compaction, and the post-compaction turn. The final E2E event was `ASTERBRIDGE_CODEX_COMPACTION_OK` with `sameOwner=true` and `itemType=contextCompaction`. The candidate runtime and its managed tunnel were stopped afterward; the unrelated DevPilot tunnels were left untouched. This closes the U5-W2 live-browser gate.
- Fixed the Windows/Linux terminal `tunnel` lifecycle exposed while cleaning up that live gate. LaunchAgent start/restart/stop is now used only on macOS; Windows/Linux use the existing native `tunnel-client runtimes connect/stop` manager directly. A successful stop no longer performs an additional native status query that can block after the alias has already exited. Focused CLI/tunnel coverage passes **21 pass / 0 fail / 74 assertions**, TypeScript PASS, and a real Windows `codex-chatgpt-web tunnel stop` now exits **0** with an explicit stopped runtime result.
...
- External Roxy/system-browser connections now mark their shared CDP transport as suspect after a proven response-DOM read failure. The failing turn still terminates normally, and concurrent turns are not disconnected; the next turn that is the sole active owner first drops the stale Playwright transport and retained page handles, then performs ordinary endpoint rediscovery instead of reusing a zombie `managedBrowserReady` connection.
- Submission acceptance no longer treats an unbound assistant locator as authoritative terminal-error evidence. Before the new assistant turn exists, that locator can overlap a historical failed response; AsterBridge now uses only current submission evidence at that boundary and checks terminal response errors after the new assistant turn is bound in the acknowledgement/observation loop.
- Current-turn terminal error detection now also recognizes ChatGPT's structural `regenerate-thread-error-button`, so localized or revised error copy still produces the existing retryable `502 upstream_server_error` instead of degrading into a later generic DOM-health timeout.
...
- Audited the later `a3a5083` helper/Launcher physical-settlement rewrite and kept it out of this slice because its cleanup ownership depends on the newer cross-process helper protocol; importing only half of that protocol would add a second lifecycle state machine to the current Roxy/system-browser production path.
- Backported the production-relevant retained-page recovery boundary from the later browser recovery work: a retained Roxy/system-browser page must now answer a bounded **5 s** DOM/CDP observation probe before reuse. A page that is not formally closed but has become unobservable is evicted and closed best-effort, so `requireRetainedConversation` fails immediately into the existing stale-source recovery path instead of hanging deeper in compaction.
- Audited the later upstream Codex-config preservation changes against AsterBridge journal v10. The current Interrupt-hook ownership already preserves unrelated TOML tables inserted by Codex while failing closed if a later table extends the owned hook/trust state or if an earlier Interrupt group changes the recorded hook order; new regressions lock those cases without importing the upstream catalog/Voice/subagent-route redesign.
- Focused validation: TypeScript PASS; browser-worker contract + retained-compaction **93 pass / 0 fail**. The helper/Launcher physical-settlement ownership additions from later upstream `a3a5083` remain intentionally separate and are not claimed complete here.
...
- A second long-thread production trace then isolated a semantic status-grounding edge after the completion fence itself was proven correct: the current image invocation returned an actual PNG and `images/edits` HTTP 200, yet the Web model's final prose still repeated an older retained-conversation claim of "3 consecutive HTTP 502". Image MCP results now carry a symmetric authoritative current-invocation annotation: success explicitly supersedes older 502/503/504 failures, while a current transient failure remains explicitly marked retryable. Historical image-backend failures can no longer be the only status cue after a successful current tool result.
- Added an image-generation provider selector with **Auto**, **Codex Tool / MCP**, and **Web Direct** modes. The setting is stored in the runtime config SSOT, routed through Launcher preload/IPC/setup, and applied with the existing safe runtime restart transaction; active Codex turns must finish or be explicitly cancelled before a provider switch.
- Web Direct is a real browser-native image path, not an alias for the Codex image tool. It opens a normal authenticated ChatGPT conversation in the configured RoxyBrowser or system-browser profile, uploads explicit reference files when present, waits for upload/send readiness, invokes ChatGPT's first-party image generation, downloads the generated image through the same authenticated browser network stack, saves the original bytes under `~/.codex/generated_images/<thread>/`, and returns that image to the tool loop. Temporary Chat is intentionally not used because live validation confirmed that first-party image generation is unavailable there.
- **Auto** preserves Codex Tool as the primary route and falls back to Web Direct only after a proven transient 502/503/504 image failure. It does not bypass 429 usage/rate limits, and it will not silently drop `num_last_images_to_include` semantics when no exact local reference paths are available. A broker local-activity fence now spans the full image route, including the native-to-Web-Direct handoff, so browser completion cannot race ahead during fallback.
- Live Windows Web Direct validation completed both a plain generation (~60.3 s) and a GTNH reference-image edit (~110.0 s) without invoking `image_gen__imagegen`; the reference edit produced a 1536×1024 PNG and the authenticated in-page fetch recovered the original 2,802,842-byte `image/png` payload with HTTP 200. Final focused coverage passes **135 / 135**, and the complete release gate passes Core **42 / 42** deterministic batches plus Launcher **217 pass / 0 fail / 1 platform-inapplicable skip** (**218 total**), TypeScript, renderer build, `RELOCATABLE_RUNTIME_SMOKE_OK`, and `git diff --check`.
...
- Final rebuilt Windows package passes `PACKAGED_LAUNCHER_SMOKE_OK win32/x64`. The local installer is **161,932,951 bytes**, SHA-256 **`8256B9EB5E5CBF5FE8E3E1A074C0293566577537EAF11CD1B8A8AE473235E8B1`**. The installed application and durable production runtime both report **3.0.36** with bundle id **`8811eac1b985f9cd78908942aca1fd23be209dd991fb118de12b7c8e39a78ec0`**. Production `/healthz` returned `status=ok`, `mode=full`, `accepting_turns=true`, and zero active HTTP/browser turns after the same-version clean reinstall; the existing private `.codex-chatgpt-web` data was preserved.
- Validation: focused provider/harness/Launcher coverage **135 pass / 0 fail**, Core **42 / 42** deterministic batches, Launcher **217 pass / 0 fail / 1 platform-inapplicable skip** (**218 total**), root and Launcher TypeScript PASS, renderer production build PASS, `RELOCATABLE_RUNTIME_SMOKE_OK`, package smoke PASS, and `git diff --check` PASS.
