...
/** One-shot capability store for compaction summaries. It never owns a Codex tool environment. */
export class CompactionTransactionStore {
  ...
  wait(token: string, signal?: AbortSignal): Promise<string> {
    ...
    return new Promise<string>((resolveWait, rejectWait) => {
      const waiter: TransactionWaiter = { resolve: resolveWait, reject: rejectWait, ...(signal ? { signal } : {}) };
      if (signal) {
