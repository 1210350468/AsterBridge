...
export interface CompiledChatGptWebPrompt {
  ...
  /** Transactional context transport. Normal prompts remain inline unless Bigger Context is enabled. */
...
export function withoutRetiredTurnHandles(contextJson: string): string {
  // Scrub decoded string values only. Replacing raw serialized JSON can mutate structural keys,
  // native call identifiers, or a handle that happens to sit beside an escaped word character.
...
export function compileChatGptWebPrompt(
  ...
  const build = (sourceMessages: readonly CodexMessage[], omittedMessages = 0): CompiledChatGptWebPrompt => {
    ...
    const activeTransportResume = omittedMessages > 0 ? [
      "<codex_transport_resume>",
  ...
  // Multipart compaction uses the same per-message staging limits as ordinary Bigger Context
  // turns. Applying the legacy single-message byte cap here would silently discard context that
  // the staged transport can carry.
  ...
  // A cumulative checkpoint can be the only surviving account of earlier work. Preserve the
  // newest one and the final compaction instruction; trim other history in original order.
