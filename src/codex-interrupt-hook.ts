...
export const MANAGED_INTERRUPT_HOOK_START =
  "# Managed by AsterBridge: release the exact Responses request when its Codex turn is interrupted.";
export const MANAGED_INTERRUPT_HOOK_END =
  "# End AsterBridge interrupt lifecycle hook.";
...
/** Match Codex's trusted command-hook identity for the normalized Interrupt hook. */
...
export function installCodexInterruptHookCommand(
  ...
  if (managedMarkerCount(text) !== 0 || text.includes(MANAGED_INTERRUPT_HOOK_END)) {
    throw new Error("Codex config already contains an AsterBridge interrupt hook marker");
  }
