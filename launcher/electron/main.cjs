...
const DEV_TRAY_ICON_PATH = path.join(__dirname, "..", "assets", "icon.ico");
...
async function trayImage() {
  ...
  // Windows notification-area icons are much more reliable when Electron receives a native ICO
  // (or the icon embedded in the packaged EXE) instead of a large SVG rasterized down to 18px.
  // This also preserves the multi-resolution icon generated by electron-builder for HiDPI trays.
  if (process.platform === "win32") {
    if (app.isPackaged) {
      const executableIcon = await app.getFileIcon(process.execPath, { size: "small" }).catch(() => null);
      if (executableIcon && !executableIcon.isEmpty()) return executableIcon;
    }
    const developmentIcon = nativeImage.createFromPath(DEV_TRAY_ICON_PATH);
    if (!developmentIcon.isEmpty()) return developmentIcon;
  }
