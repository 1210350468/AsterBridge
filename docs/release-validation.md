...
8. Configure a fresh `Codex Native3` connector, run **Verify runtime**, and complete one Full-mode harmless local tool turn. Confirm that retired `Codex Native` / `Codex Native2` identities produce an explicit migration error rather than being silently reused. Repeat with Pro when the account exposes Pro. Then start a fresh Full-mode Codex task whose outer registry exposes `image_gen__imagegen`, ask Temporary Chat to generate a harmless image, and prove the launcher log contains the broker `image_gen__imagegen` call plus `[asterbridge:image]` telemetry; a ChatGPT first-party image tool result without those two pieces of evidence is a release failure. Confirm the produced artifact exists under Codex `generated_images`. Attach eight real images to a separate Web turn and prove all eight tiles complete, the adaptive attachment/send budgets are logged, submission is accepted without a duplicate retry, and the answer completes. Finally verify direct visual understanding, close every Temporary Chat page, and prove the resumed Codex thread can still answer a previously unstated visual detail from a historical image without attaching it again.
