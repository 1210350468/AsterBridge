...
Current status: **AsterBridge 3.0.25 returns the product to its original Full-Harness contract: MCP/`Codex Native3` is the primary tool transport, while the connector-independent Responses bridge remains an explicit experimental fallback only. The 3.0.15-3.0.19 reliability work—automatic compaction, native provider switch, managed model catalog, eight-image handling, cancellation, daemon-restart continuity, reversible route gates, RoxyBrowser lifecycle, and native image generation—remains intact. The 3.0.24 Plus-account Responses shell/image E2E is preserved as fallback evidence, but it no longer substitutes for the stable MCP gate. Publication requires exact-source verification plus a real `Codex Native3` MCP Full-Harness revalidation on an account/workspace that exposes the custom App.**
...
Installed 3.0.13 then exposed one final Lexical transport edge: a Full Harness prompt could retain identical semantics while ChatGPT canonicalized Unicode representation, shortening the observed composer text by two UTF-16 code units and tripping exact insertion verification before the image tool ran. AsterBridge 3.0.14 accepts only NFC canonical equivalence and U+FE0E/U+FE0F presentation-selector normalization while keeping ordinary mutations fail-closed. Focused Browser Worker regression is **77 pass, 0 fail / 360 assertions**; full core remained **41/41 deterministic batches PASS**, Launcher **203 pass / 0 fail / 1 Windows-inapplicable skip**, and `RELOCATABLE_RUNTIME_SMOKE_OK`. The packaged Windows 3.0.14 runtime is installed locally with route active and `errors=[]`, Doctor `ready`, and `Codex Native3` verified. A fresh image turn queued `image_gen__imagegen`, wrote an 835,896-byte PNG, and logged `[asterbridge:image] endpoint=images/generations durationMs=15793 status=200 origin=upstream retryable=false`. A real eight-image turn used `uploadTimeoutMs=220000` / `sendTimeoutMs=104000`, accepted submission after 18.098 seconds, returned `MULTI8_OK`, and final `/healthz` reported zero active HTTP/browser turns.

3.0.15 extends prompt-integrity normalization to the CRLF/LF representation drift reproduced in a six-thread installed stress run. 3.0.16 makes routed-model metadata deterministic for Codex 0.149+ by transactionally installing an AsterBridge-owned, hash-verified `model_catalog_json` alongside `openai_base_url`, while preserving and restoring the user's original catalog assignment. 3.0.17 fixes Web-compacted threads switching back to native Sol: AsterBridge-owned `ocx1:` compaction and `ocxr1:` reasoning envelopes are converted to readable replay context, while genuine OpenAI encrypted items keep their provider-owned ids and ciphertext unchanged. 3.0.18 then replaces the old fixed 20k retained-history allowance with a route-aware post-compaction budget: it reserves the measured/observed stable harness, actual summary tokens, structure, and post-compact headroom against the selected Web model's real `auto_compact_token_limit`; historical images consume the same budget. 3.0.19 closes the Codex 0.150 protocol mismatch exposed by installed testing: the reversible v9 route pins `remote_compaction_v2=false` while connected so Codex keeps using remote compaction through bounded `/responses/compact` v1 rather than the v2 client path that can retain up to 64k history tokens. Oversized latest-user compaction appendices are also bounded to 1,024 GPT-5 tokens. A real Instant thread crossed the 31,980-token gate at `lastInput=34,760`; the next turn emitted `compacted` + `ContextCompaction` and continued at `17,639 / 31,980`, then switched to native Sol and survived a daemon restart. A real eight-image High turn completed at 51,487 input tokens; forced client cancellation returned both activity counters to zero; route disconnect/reconnect restored and reinstalled the managed config exactly. Final source verification remained **41/41 core files PASS**, Launcher **203 pass / 0 fail / 1 Windows-inapplicable skip**, TypeScript/renderer build PASS, and `RELOCATABLE_RUNTIME_SMOKE_OK`. The only remaining stable-release blocker is current ChatGPT account-side visibility of the `Codex Native3` custom MCP app, which prevents a fresh Full Harness/image-generation E2E from being re-proven. Version 3.0.5 remains the current public binary until a newer GitHub release is explicitly published.
