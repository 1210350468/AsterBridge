...
- the remaining Launcher lifecycle/update transaction deltas from the later `a3a5083` / v5.0.5-v5.0.6 evolution; helper physical-settlement ownership, retained-surface prompt selection, MCP progress/completion-fence forwarding, and false-green tunnel MCP startup gating are now backported separately rather than by importing the broader rewrite;
- exact in-turn Launcher observation rebind is now implemented. After submission, a stalled DOM probe closes the stale Playwright connection before reconnecting to the same descriptor-v3 target, with at most two rebinds and no prompt replay; submission acceptance, multipart acknowledgement, and final-response observation all use the same bounded recovery contract;
...
Gate: the exact Interrupt/config-preservation slices, 3.0.40/3.0.41 Roxy/browser hardening, and the 3.0.42 helper physical-settlement/progress/retained-compaction/live-page-rebind slices have passed their focused gates. Keep U5-W2 **IN PROGRESS** only for the remaining Launcher lifecycle/update transaction deltas that still apply to AsterBridge's Windows-first runtime.
...
3.0.42 WIP now carries physical-settlement ownership and ordinary Launcher retained-conversation continuity from the later `a3a5083` lineage without importing its unrelated Zero Risk rewrite. `ChatGptTurnRuntime` / `ChatGptTurnSession` distinguish logical browser completion from physical cleanup, compaction duplicate ownership remains held until every registered physical settlement completes, and the helper sends a separate `settled` frame after turn cleanup. `ChatGptBrowserWorker.run()` remains the stable execution/test injection boundary; production promises are paired with their true settlement while injected fakes fall back to logical completion. The Launcher helper mirrors daemon-owned MCP progress, causally acknowledges observed tool batches, and round-trips broker completion-fence begin/commit before accepting DOM completion. Structured retained compaction is enabled for Launcher, descriptor v3 publishes exact native target identities, and accepted turns now have bounded same-page recovery: a stalled observation can reconnect twice to that exact task tab only after the stale transport closes, without replaying Send. The latest widened gate is **185 pass / 0 fail / 8 existing Windows skips / 959 assertions**, TypeScript PASS, plus a successful Launcher production build. U5-W2 remains **IN PROGRESS** only for remaining applicable Launcher lifecycle/update transaction deltas.
