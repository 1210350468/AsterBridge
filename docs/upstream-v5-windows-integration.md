...
- the remaining Launcher lifecycle/update transaction deltas from the later `a3a5083` / v5.0.5-v5.0.6 evolution; helper physical-settlement ownership, retained-surface prompt selection, and MCP progress/completion-fence forwarding are now backported separately rather than by importing the broader rewrite;
