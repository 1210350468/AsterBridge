...
- the actual bounded in-turn Launcher observation rebind from v5.0.5/v5.0.6 remains to be wired, but its native target prerequisite is now present: descriptor v3 publishes exact `surfaceId -> CDP targetId` ownership, so reacquisition no longer depends on executing JavaScript in a stalled renderer;
...
Gate: the exact Interrupt/config-preservation slices, 3.0.40/3.0.41 Roxy/browser hardening, and the 3.0.42 helper physical-settlement/progress slices have passed their focused gates. Keep U5-W2 **IN PROGRESS** until exact live-page rebind and the remaining Launcher lifecycle/update transaction deltas are separately audited and validated.
...
3.0.42 WIP now carries physical-settlement ownership and ordinary Launcher retained-conversation continuity from the later `a3a5083` lineage without importing its unrelated Zero Risk rewrite. `ChatGptTurnRuntime` / `ChatGptTurnSession` distinguish logical browser completion from physical cleanup, compaction duplicate ownership remains held until every registered physical settlement completes, and the helper sends a separate `settled` frame after turn cleanup. `ChatGptBrowserWorker.run()` remains the stable execution/test injection boundary; production promises are paired with their true settlement while injected fakes fall back to logical completion. The Launcher helper now also mirrors daemon-owned MCP progress, causally acknowledges observed tool batches, and round-trips broker completion-fence begin/commit before accepting DOM completion. Structured retained compaction is enabled for Launcher: active sources must physically settle before the handoff can lease the same retained surface, and the handoff itself retains ownership until the helper's physical settlement completes. Descriptor v3 now publishes exact native target identities for every Launcher-owned browser surface, removing renderer-JavaScript discovery from reacquisition and providing the prerequisite for bounded same-page rebind. U5-W2 remains **IN PROGRESS** until that actual rebind loop and the remaining Launcher lifecycle/update transaction deltas are separately validated.
