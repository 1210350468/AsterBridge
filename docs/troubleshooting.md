...
| `Refusing to stop launcher-owned runtime because atomic idleness could not be proven` while changing Bigger Context | AsterBridge proved that at least one HTTP/browser turn was still active, so it refused to restart the runtime underneath that task | Current Launcher builds preflight the activity counters and let you keep the turn or explicitly cancel it and switch. On older builds, finish/cancel the active Codex turn first, then retry the setting; do not force-kill the runtime. |
...
| A native model such as `gpt-6-astra` disappears while the bridge is active | A fresh provider cache can lag the current Codex Desktop bundled catalog | Refresh the active bridge. Current builds preserve cache metadata for known models and supplement missing visible slugs from the current Desktop bundle, so local-gateway refresh no longer drops bundled models merely because the cache is incomplete |
