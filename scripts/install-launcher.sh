...
REPOSITORY="${CODEX_WEB_GPT_REPOSITORY:-1210350468/AsterBridge}"
...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest AsterBridge release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="asterbridge-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
...
if [ "$OS" = "Darwin" ]; then
  ...
  SOURCE_APP="$STAGE_DIR/AsterBridge.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/AsterBridge" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/AsterBridge.app"
  LEGACY_APP="$INSTALL_DIR/Codex Web GPT.app"
  if pgrep -x "AsterBridge" >/dev/null 2>&1 || pgrep -x "Codex Web GPT" >/dev/null 2>&1; then
    echo "Quit AsterBridge before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/AsterBridge.previous.app"
  if [ -e "$TARGET_APP" ]; then mv "$TARGET_APP" "$BACKUP_APP";
  elif [ -e "$LEGACY_APP" ]; then mv "$LEGACY_APP" "$BACKUP_APP"; fi
...
TARGET="$TARGET_DIR/AsterBridge.AppImage"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "AsterBridge\\.AppImage" >/dev/null 2>&1 \
...
Version=1.0
Name=AsterBridge
Comment=Local bridge from Codex to ChatGPT Web and native tools
