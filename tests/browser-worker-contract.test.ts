...
test("browser turn orchestration retains owned prompt insertion and semantic submission", () => {
  ...
  expect(runBrowserTurn).toContain("submissionBaseline,");
  expect(runBrowserTurn).toContain("recoverLauncherObservation,");
...
test("launcher DOM recovery is bounded and never opens a replacement before the stale connection closes", async () => {
  expect(CHATGPT_BROWSER_OBSERVATION_PROBE_TIMEOUT_MS).toBe(5_000);
  expect(MAX_CHATGPT_BROWSER_PAGE_REBINDS).toBe(2);
  await expect(withChatGptBrowserObservationTimeout(new Promise<never>(() => {}), 5))
...
test("an accepted send survives one stalled observation without pressing Send twice", async () => {
  type Baseline = {
    userTurns: unknown;
  ...
  const firstPage = { locator: () => hidden } as unknown as Page;
  const reboundPage = { locator: () => hidden } as unknown as Page;
  let sendPresses = 0;
  const sendButton = {
    waitFor: async () => {},
  ...
  worker.currentSubmissionEvidence = async () => {
    observations += 1;
    if (observations === 1) throw new ChatGptBrowserObservationTimeoutError(5_000);
    return "user_turn";
  };
  ...
  let recoveries = 0;
  await expect(worker.sendAttachedPrompt(
    firstPage,
    baseline,
    undefined,
    undefined,
    async (attempt, cause, observedBaseline) => {
      ...
      return { page: reboundPage, baseline };
    },
...
test("accepted-send observation recovery stops after two exact-page rebinds", async () => {
  type Baseline = {
    userTurns: unknown;
    responseTurns: unknown;
    initialUserTurnCount: number;
  ...
  const worker = ChatGptBrowserWorker.forProvider({
    adapter: "chatgpt-web",
    baseUrl: `browser://launcher-rebind-budget-${Date.now()}-${Math.random()}`,
    chatgptWeb: { localToolsEnabled: false, solAvailable: true, proAvailable: true },
  }) as unknown as {
    currentSubmissionEvidence(): Promise<never>;
  ...
  const page = { locator: () => hidden } as unknown as Page;
  const baseline: Baseline = {
    userTurns: {},
    responseTurns: {},
    initialUserTurnCount: 0,
  ...
  )).rejects.toThrow("DOM remained unresponsive after 2 same-page rebinds");
...
test("retained external pages must still answer a bounded DOM probe before reuse", async () => {
  await expect(chatGptRetainedPageIsObservable({
    isClosed: () => false,
    evaluate: async () => "complete",
  } as never, 25)).resolves.toBeTrue();

  await expect(chatGptRetainedPageIsObservable({
    isClosed: () => true,
    evaluate: async () => { throw new Error("must not evaluate a closed page"); },
  } as never, 25)).resolves.toBeFalse();
...
test("connector verification reports a legacy-only ChatGPT menu as a migration error", async () => {
  ...
  expect(mixedMessage).toContain('Legacy ChatGPT connector "Codex Native" was found');
  expect(mixedMessage).toContain('newly created connector named "Codex Native3"');

  const native2Message = await connectorMentionFailure.call({
    config: { appName: CHATGPT_CONNECTOR_NAME },
    connectorMentionRowTitles: async () => ["Codex Native2"],
  }, {}, 4);
  expect(native2Message).toContain('Legacy ChatGPT connector "Codex Native2" was found');
  expect(native2Message).toContain('newly created connector named "Codex Native3"');
});
...
test("prompt attachment integrity failures are local non-retryable bridge errors", () => {
  const error = new ChatGptPromptAttachmentIntegrityError("composer truncated a prompt chunk");
  expect(error.status).toBe(500);
  expect(error.errorType).toBe("proxy_error");
  expect(error.code).toBe("prompt_attachment_integrity_error");
...
test("large read-only context is inserted as contiguous bounded edits before exact verification", async () => {
  ...
  const committed = calls.filter(call => call[0] === "chunkCommitted");
  expect(committed).toHaveLength(inserted.length);
  expect(committed.at(-1)).toEqual(["chunkCommitted", String(prompt.length)]);
  expect(calls.filter(call => call[0] === "reanchor")).toHaveLength(inserted.length - 1);
...
test("multi-chunk prompt insertion repairs a drifted Lexical caret after each exact prefix", async () => {
  ...
  expect(calls).toEqual([
    ...
    ["reanchor"],
    ["insertText", "457"],
    ["chunkCommitted", String(CHATGPT_PROMPT_INSERT_CHUNK_CHARS + 457)],
  ]);
});

test("prompt insertion retries one proven no-op Lexical chunk without duplicating context", async () => {
  const prompt = "x".repeat(CHATGPT_PROMPT_INSERT_CHUNK_CHARS * 3 + 123);
  let attached = "";
  let swallowed = false;
  let insertCalls = 0;
  const page = {
    keyboard: {
      insertText: async (value: string) => {
        insertCalls += 1;
        if (!swallowed && attached.length === CHATGPT_PROMPT_INSERT_CHUNK_CHARS) {
          swallowed = true;
          return;
        }
        attached += value;
    ...
    waitForPromptChunkAttached: async (_page: unknown, expected: string) => {
      if (attached !== expected) {
        throw new ChatGptPromptAttachmentIntegrityError(
          `chunk mismatch expected=${expected.length} actual=${attached.length}`,
        );
      }
    },
  ...
  expect(insertCalls).toBe(Math.ceil(prompt.length / CHATGPT_PROMPT_INSERT_CHUNK_CHARS) + 1);
  expect(attached).toBe(prompt);
});

test("prompt insertion never retries a partially committed Lexical chunk", async () => {
  const prompt = "x".repeat(CHATGPT_PROMPT_INSERT_CHUNK_CHARS * 2 + 123);
  let attached = "";
  let insertCalls = 0;
  const page = {
    keyboard: {
      insertText: async (value: string) => {
        insertCalls += 1;
        if (attached.length === CHATGPT_PROMPT_INSERT_CHUNK_CHARS) {
          attached += value.slice(0, 100);
          return;
        }
        attached += value;
      },
  ...
  await expect(insertPromptText.call({
    ...
    promptTextEquivalent: (expected: string, observed: string) => expected === observed,
    reanchorPromptCaret: async () => {},
  }, page, prompt)).rejects.toBeInstanceOf(ChatGptPromptAttachmentIntegrityError);
  expect(insertCalls).toBe(2);
  expect(attached.length).toBe(CHATGPT_PROMPT_INSERT_CHUNK_CHARS + 100);
});
...
test("the real compaction envelope survives simulated caret drift at every bounded edit boundary", async () => {
  ...
  expect(simulatedDrifts).toBe(nativeEdits);
...
test("submission acceptance stops when its stage is aborted", async () => {
  ...
  await expect(waitForSubmissionAccepted.call(
    {},
    {} as Page,
    { userTurns: {}, responseTurns: {}, initialUserTurnCount: 0, initialResponseTurnCount: 0 },
    controller.signal,
